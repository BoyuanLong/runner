...
def run_slurm(run_description, args):
    ...
    for sbatch_file in sbatch_files:
        ...
        cmd = f'sbatch --gres=gpu:{args.slurm_gpu_type}:{args.slurm_gpus_per_job} --qos {args.slurm_qos} -c {num_cpus} --parsable --output {workdir}/{sbatch_fname}-slurm-%j.out {sbatch_file}'
